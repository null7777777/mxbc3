...
<head>
	...
	<link rel="stylesheet" href="css/tea/head_footer.css" >
	<link rel="stylesheet" href="css/tea/tealist.css" />
								...
								<ul class="row"> 
									<c:choose>
										<c:when test="${!empty teaList}">
											<c:forEach items="${teaList}" var="i">
												<li class="col-md-3">
													<div class="list">
														<a href="teadetail?teaId=${i.teaId}">
															<img class="img-responsive" src="${i.upLoadImg.imgSrc}" />
														</a>
														<div class="proinfo">
															<h2>
																<a style="margin-left:10px;color:#333333;" href="teadetail?teaId=${i.teaId}">${i.teaName}</a>
															</h2>
															<p>
																<i>￥${i.price}</i>
																<button class="btn btn-xs" style="background:#333333; color:#fff;float:right; " onclick="addToCart(${i.teaId})" href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-sm">
																<img class="cart-icon" src="images/tea/btn-cart-icon.png" alt="">
																购买</button>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}TeaList?action=list&page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}TeaList?action=list&page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){
		
		window.location="${basePath}TeaList?action=list&page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		window.location="${basePath}TeaList?action=list&page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}TeaList?action=list&page="+page;
		}
	}) 
